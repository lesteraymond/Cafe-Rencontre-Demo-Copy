...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        ...
        switch ($action) {
            ...
            case 'low_stock':
                // Get low stock warnings (percentage = usage, higher = more consumed)
                // CRITICAL: >= 90% used, LOW: >= 80% used
                ...
                $lowStock = [];
                while ($row = $result->fetch_assoc()) {
                    $row['status'] = $row['percentage'] >= 90 ? 'CRITICAL' : 'LOW';
                    $lowStock[] = $row;
                }
            ...
            case 'status':
            default:
                // Get full inventory status (percentage = usage, higher = more consumed)
                $result = $conn->query("SELECT *,
                                        CASE WHEN bottles_count > 0 THEN ROUND(100 - (available_quantity / bottles_count) * 100) ELSE 0 END as percentage,
                                        CASE 
                                            WHEN bottles_count > 0 AND (100 - (available_quantity / bottles_count) * 100) >= 90 THEN 'CRITICAL'
                                            WHEN bottles_count > 0 AND (100 - (available_quantity / bottles_count) * 100) >= 80 THEN 'LOW'
                                            WHEN bottles_count > 0 AND (100 - (available_quantity / bottles_count) * 100) >= 50 THEN 'MEDIUM'
                                            ELSE 'OK'
                                        END as stock_status
                                        FROM ingredients
                                        ORDER BY percentage DESC");
